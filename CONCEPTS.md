...
The "shorter / lexicographically smaller path wins" rule can be overridden using the `--prefer` and `--ignore` options, which accept absolute filesystem paths with shell-like pattern syntax (understanding `?`, `*`, `**`, `[a-z]`, `{this,that}`).
